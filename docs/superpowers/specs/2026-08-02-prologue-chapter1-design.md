...
| # | 목표 | 목표 타입 | 게이팅하는 시스템 |
|---|---|---|---|
| 1 | 착지 지점을 벗어나 주변을 살핀다 | `ReachZoneObjective` | Locomotion |
| 2 | **발광 버섯 군락**을 찾는다 | `ReachZoneObjective` | `OxygenZone` — 안전 거점 개념 학습 |
| 3 | **스크랩 12개**를 모은다 | `CollectItemObjective` | Harvesting + 약한 전투(눈·공) |
| 4 | **산소 필터**를 제작한다 | `CraftItemObjective` | Crafting — 산소 감소율 완화 |
| 5 | **강을 건너 반대편에 도달**한다 | `ReachZoneObjective` | 열매게 서식지, 강가 자원 |
| 6 | **포탈을 기동**한다 | `InteractObjective` | `PortalDevice` — 부품 납품 후 하강 |

우주복 필터 손상으로 착지 직후부터 산소가 지속 감소한다. 목표 4를 마치면 감소율이 완화되어 탐사 범위가 넓어진다.
