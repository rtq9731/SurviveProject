...
namespace Survive.Testing
{
    ...
    ///
    /// <b>배선은 그 뒤에 섰다 (2026-08-07).</b> 이 주석은 원래
    /// "<see cref="ScytheFsm"/>을 부르는 런타임 코드가 하나도 없다"였는데,
    /// 지금은 <c>Creatures/ScytheMind</c>가 매 프레임 그것을 부르고
    /// <c>CreatureBrain.Awake</c>가 부유하는 개체마다 자동으로 붙인다.
    ///
    /// <b>그래도 이 시나리오는 규칙을 직접 묻는다.</b> 옮기지 않은 이유가 있다 —
    /// 여기서 재는 것은 <b>세계가 만든 상황이 규칙에 무엇을 시키는가</b>이고,
    /// 살아 있는 개체를 통해 물으면 그 개체가 어디에 있느냐(서식지·거리·시각)가
    /// 답에 섞인다. 실제 개체 위에서의 확인은 <c>E2EScytheFsm</c>이 따로 한다.
    /// 이 둘은 겹치는 것이 아니라 <b>같은 규칙을 양쪽에서</b> 누르는 것이다.
    /// </summary>
