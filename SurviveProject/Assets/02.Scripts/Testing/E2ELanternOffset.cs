...
namespace Survive.Testing
{
    ...
    public static class E2ELanternOffset
    {
        ...
        static IEnumerator 낫이_등_뒤로만_파고든다()
        {
            ...
            // 조명탄이 존재할 이유가 사라진다.
            //
            // 완전히 0은 아니다. 사람이 몸을 다시 잡는 사이 낫이 정확히 옆으로
            // 돌아가는 프레임이 섞이고, 그때는 내준 쪽이 아니라 빛이 민다 —
            // 그것은 규칙이 제대로 도는 증거이지 새는 것이 아니다. 그래서 <b>비율</b>로 본다.
            E2EHarness.Assert(등판.물러난프레임 * 10 < 등판.빛속프레임,
                              $"등지면 랜턴이 붙은 것을 떼어내지 못한다 " +
                              $"(빛 안 {등판.빛속프레임}프레임 중 물러난 것은 {등판.물러난프레임}프레임뿐)");
