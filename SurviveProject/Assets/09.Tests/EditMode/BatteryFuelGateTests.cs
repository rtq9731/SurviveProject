...
public class BatteryFuelGateTests
{
    ...
    ///
    /// <b>「고강도」만 따로 찾지 않는 이유.</b> 외계 합금 설명문이 이미 그 낱말을
    /// 쓴다("화성인이 만든 고강도 구조재"). 폐기된 것은 낱말이 아니라
    ...
    /// 비어 돌지 않는다. (A섬에 서 있던 채집물은 2026-08-07에 「합금 더미」로
    /// 개명했다. 매크로늄이 아니라 외계 합금을 떨구는 물건이었다.)
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// <b>A섬은 스크랩과 기계 부품만이다.</b> 새 물질은 B섬부터 — 그 재료를 A-2·A-3에
    /// 둔 것은 "강 건너로 갈 이유가 필요하다"는 걱정에서 나왔는데, A-3에는 이미
    ...
    public void 매크로늄을_떨구는_채집물은_아직_없다()
    {
        ...
        Assert.IsEmpty(떨구는것,
            "매크로늄을 캘 수 있는 것이 생겼다. 그 자리가 A섬이면 회신 ①과 어긋난다 " +
            "(A섬은 학습·준비 구간이고 새 물질은 B섬부터다):\n  " +
            string.Join("\n  ", 떨구는것));
    }
