...
public class ResearchGatingTests
{
    ...
    {
        var e = 항목("res_breach_pod", 소재("relic_core", "낫의 핵", 1));
    ...
    {
        var 핵 = 항목("res_breach_pod", 소재("relic_core", "낫의 핵", 1));
    ...
    public void 목록은_겪은_것을_따라_자란다()
    {
        var 책 = new[]
        {
            ...
            항목("res_breach_pod", 소재("relic_core", "낫의 핵",   1)),
        };
    ...
    public void 다_써_버려도_기록은_남는다()
    {
        ...
        var e = 항목("res_breach_pod", 소재(핵, 1));
    ...
    public void 저장을_왕복해도_가져_본_기록이_남는다()
    {
        ...
        Assert.IsTrue(MenuListing.ShouldList(항목("res_breach_pod", 소재("relic_core", "낫의 핵", 1)), 다시));
    }
    ...
    public void 습득_기록_열쇠는_청사진_도감_열쇠와_겹치지_않는다()
    {
        ...
        StringAssert.StartsWith(HeldRecord.Prefix, HeldRecord.KeyFor("bp_breach_pod"));

        HeldRecord.Record(_ledger, "bp_breach_pod");
        Assert.IsFalse(_ledger.IsUnlocked("bp_breach_pod"),
    ...
    {
        var book = 연구목록();
        var 핵항목 = book.entries.First(e => e != null && e.id == "res_breach_pod");
        ...
        CollectionAssert.AreEquivalent(new[] { "res_breach_pod" }, 실린것);
