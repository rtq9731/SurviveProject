...
///
/// <b>왜 필요한가.</b> 2026-08-06에 짙어진 매크로늄 구간을 뚫고 내려가는 탈것의 이름을
...
public class RenamedNameGateTests
{
    ...
    static readonly string[] 옛이름 =
    {
        ...
        // 지상에 서 있던 채집물의 옛 이름(2026-08-07 개명). 여기만은 <b>구절 전체</b>를
    ...
    // ── 지상 채집물의 개명 (2026-08-07 회신 ①) ───────────────────
    //
    // 회신 ①이 지상에서 매크로늄을 없앴다. 지상은 스크랩과 기계 부품만 나오는
    // 학습·준비 구간이고 새 물질은 지하부터다. 그런데 씬에는 매크로늄의
    // 이름을 단 채집물이 열세 곳 서 있었다 — 그것이 실제로 떨구는 것은
    // 매크로늄이 아니라 <b>이종 합금</b>인데도.
    //
    ...
    // ── 그 채집물이 떨구는 물건의 개명 (2026-08-07 세계관 §8) ────
    //
    // 「외계」가 아니다. 갖고 도망친 것이라 인류의 기반기술과 닮았고, 다만 지구
    // 환경에서 만들어온 것이라 성분이 다르다. 그래서 새 이름은 그 <b>다름</b>만
    // 말한다 — 떨구는 쪽(「합금 더미」)과도 구별된다.
    //
    // id(<c>alien_alloy</c>)는 그대로 둔다. 열아홉 파일 마흔 자리가 물고 있고,
    // 화면에 나가지 않는다 (채집물 개명과 같은 판단).

    const string 합금아이템 = "Assets/08.Data/Items/이종 합금.asset";
    const string 새합금이름 = "이종 합금";

    /// <summary>
    /// <b>바꾼 이름이 조회 경로 끝에서도 새 이름이다.</b> 에셋만 고치면 화면은
    /// 안 바뀐다 — 표가 있으면 표가 이기기 때문이다. 채집물 쪽과 같은 확인이지만
    /// 이쪽은 <b>파일 이름까지</b> 함께 옮겼다는 것도 본다. 에셋의 <c>m_Name</c>은
    /// 파일 이름을 따라가므로, 파일을 두고 이름만 고치면 폐기어가 파일 안에 남는다.
    /// </summary>
    [Test]
    public void 합금의_새_이름이_에셋에도_표에도_있다()
    {
        var 아이템 = AssetDatabase.LoadAssetAtPath<ItemDataSO>(합금아이템);
        Assert.IsNotNull(아이템, 합금아이템 + "를 못 읽었다 (파일 이름을 같이 옮겼는가)");

        Assert.AreEqual("alien_alloy", 아이템.id,
            "id까지 바꿨다. 세이브와 열아홉 파일이 이 문자열을 물고 있다");
        Assert.AreEqual(새합금이름, 아이템.displayName,
            "에셋 원문이 새 이름이 아니다. 표가 없는 배포본에서는 이 값이 그대로 화면에 나간다");
        Assert.AreEqual(새합금이름, DataText.Name(아이템),
            "표를 거친 값이 새 이름이 아니다 — Tools/Survive/번역/데이터 에셋 초안 갱신 을 돌렸는가");

        Assert.AreNotEqual(새채집물이름, 새합금이름,
            "떨구는 채집물과 떨어지는 물건의 이름이 같아졌다");
    }
