...
public class RetiredContentGateTests
{
        ...
        폐기된_세계관 =
    {
        ...
        // 2026-08-07 3 -> 2 -> 0. 마지막 하나는 AiVoice.HasEmoji의 문서 주석이
        // 폐기된 프롤로그 자막("$#@%급 …")을 본보기로 들고 있던 것이었다.
        // 본보기를 지금 화면에 나가는 문장으로 갈았다.
        //
        // <b>낱말이 0이 되어도 무대는 따로 봐야 한다.</b> 프롤로그 씬의 파티클
        // 이름은 라틴 글자여서 이 훑개에 안 걸리고, 게이트가 초록인 채로 폐기된
        // 기상이 첫 화면에서 돌 수 있었다 — 실제로 그랬다. 그쪽은
        // PrologueRetirementTests가 씬 파일을 직접 훑어 막는다
        ("지표를 덮은 기상",     new[] { "모래" + "폭풍", "모래 " + "폭풍" },     false,  0),
        ...
        //
        // <b>2026-08-07 4 -> 0. 이 갈래도 이제 진짜 게이트다.</b> 마지막 넷은 낫 갈래가
        // 들고 있던 주석이었고(<c>Creatures/HoverDrifter</c>와 <c>Domain/Creatures/</c> 셋),
        // 그 라운드가 새 어휘로 다시 썼다 — 안쪽 섬을 가리키던 말은 그것이 실제로
        // <c>SurfaceZone.Inland</c>였으므로 <b>안쪽 뭍</b>으로, 건너편 섬 도착은
        // <b>먼 뭍 도착</b>으로. 여기 옛 이름을 그대로 적으면 이 파일이 제 검사에 걸린다.
        ("군도 구역 이름",       new[] { "A" + "섬", "B" + "섬" },               false,  0),
    ...
    // ── ⑤ 라틴 글자로 된 이름 (낱말 게이트의 사각지대) ──────────
    //
    // 위의 게이트는 <b>글</b>을 찾는다. 폐기된 개념은 한글로 적히므로 한글
    // 금지어로 잡힌다. 그런데 씬과 프리팹의 <b>오브젝트 이름</b>은 라틴 글자라
    // 한글 금지어에 걸릴 수가 없다 — 원리적으로 못 본다.
    //
    // <b>실제로 사고가 있었다.</b> 폐기된 기상의 파티클 넷이 빌드 인덱스 0인 씬
    // 안에 들어 있었고, 그 안에 초당 8씩 산소를 깎는 트리거 넷이 살아 있었다.
    // 이름이 라틴 글자여서 게이트는 초록이었다. 그때 사람이 씬 하나를 손으로 훑어
    // 막고 이렇게 적어 두었다 — "같은 종류의 구멍이 다른 씬·프리팹에 더 있을 수 있다."
    //
    // 여기가 그 구멍을 세는 자리다. 이번에는 사람 눈이 아니라 기계가 센다.

    /// <summary>
    /// <b>폐기된 개념을 가리키는 라틴 이름.</b> 위의 <c>폐기된_세계관</c> 표와 짝이고,
    /// 다른 점은 <b>무엇을 훑는가</b>뿐이다 — 저쪽은 본문, 이쪽은 오브젝트 이름.
    ///
    /// <b>왜 0이 아니라 상한인가.</b> 위의 상한 표와 같은 이유에 하나가 더 붙는다.
    /// 이름을 고치는 것은 씬·프리팹을 고치는 일인데 그것은 병합할 수 없는 파일이고,
    /// 무엇보다 <c>Archipelago</c> 프리팹은 <b>오늘 살아 있는 E2E가 이름으로
    /// 집는다</b>(<c>E2ELandmass</c>·<c>E2ETuningSurvey</c>). 지금 0을 요구하면
    /// 폐기된 무대가 아니라 <b>멀쩡한 검증</b>이 먼저 빨개진다. 세고 막는 것이
    /// 먼저이고, 고치는 것은 지형을 다시 만드는 라운드의 일이다(스펙 §13).
    ///
    /// <b>여기 없는 것과 그 이유.</b>
    /// <list type="bullet">
    /// <item><c>Surface</c> — 살아 있다. 「지표를 덮은 것은 매크로늄 바다다」가
    ///       정본의 문장이고(세계관 §8), <c>SurfaceZone</c>은 이번 무대의 구역
    ///       열거형이다. <c>GlobalVolume_Surface</c>는 <b>갈색 볼륨이라서</b>
    ///       폐기 대상이지 이름 때문이 아니다 — 그것은 눈으로 갈라야 하는 일이라
    ///       낱말 게이트의 몫이 아니다</item>
    /// <item><c>Shoal</c>·<c>Shallows</c> — 「여울」은 살아 있는 구역이다
    ///       (<c>SurfaceZone.Shallows</c>). <c>Island2_Shoal</c>이 폐기인 것은
    ///       <c>Shoal</c> 때문이 아니라 앞의 <c>Island</c> 때문이다</item>
    /// <item><c>Alien</c> — 표시 이름만 폐기고 <c>alien_alloy</c>는 살아 있다.
    ///       이름 쪽에 남은 <c>Drop_AlienAlloy_Mat</c>은 그 아이디를 따른 것이다</item>
    /// <item><c>Landing</c> — 스스로 측량하고 내려앉았다(세계관 §6). 폐기된 것은
    ///       <b>떨어져서 도착했다</b>는 사실 쪽이라 <c>Crushed</c>를 잡는다</item>
    /// </list>
    ///
    /// <b>말을 조각으로 잇는 것은 위 표와 같은 이유다</b> — 이 파일도 저장소
    /// 안이고, 언젠가 훑는 자리에 <c>*.cs</c>가 더해지는 날 예외 목록을 만들지
    /// 않으려면 지금부터 잇는 편이 싸다.
    /// </summary>
    static readonly (string 갈래, string[] 말들, int 지금_남은_자리)[] 폐기된_라틴_이름 =
    {
        // 무대는 지상 한 덩어리 + 호수 + 착륙선이다 (세계관 §8).
        ("군도라는 지형",      new[] { "Archi" + "pelago" }, 2),
        ("섬이라는 무대",      new[] { "Is" + "land" },      12),
        ("떠 있는 땅덩이",     new[] { "Float" + "ingIsland" }, 1),

        // 지하는 목적지이지 무대가 아니다. 프롤로그도 동굴로 피하지 않는다
        // (상세기획서 §6.1). Update 1의 지하 동굴이 실제로 생기는 날에는
        // 그것이 살아 있는 개념이므로 상한을 올리고 그 이유를 여기 적는다.
        ("동굴이라는 무대",    new[] { "Cave" },             4),

        // 스스로 측량하고 골라서 내려앉았다 (세계관 §6).
        ("떨어져 부서진 도착", new[] { "Crush" + "ed", "Cras" + "h", "Wrec" + "k" }, 1),

        // 지표를 덮은 것은 매크로늄 바다다 (세계관 §8). 프롤로그 라운드가
        // 파티클 넷을 걷어내 0이 됐고, 이 줄이 그 0을 지킨다.
        ("지표를 덮은 기상",   new[] { "Dust" + "Storm", "Sand" + "Storm" }, 0),
    };

    /// <summary>
    /// <b>이쪽이 진짜 게이트다.</b> 위의 본문 게이트와 달리 여기에는 꺼 둔 짝이 없다 —
    /// 처음부터 상한으로 세우므로 오늘 초록이고, 하나라도 늘면 그날 빨개진다.
    /// </summary>
    [Test]
    public void 폐기된_라틴_이름이_지금보다_더_늘지_않는다()
    {
        var 늘어난것 = new List<string>();

        foreach (var (갈래, 말들, 지금) in 폐기된_라틴_이름)
        {
            var 자리들 = 이름이_걸린_자리(말들);
            if (자리들.Count > 지금)
                늘어난것.Add($"[{갈래}] {지금}개였는데 {자리들.Count}개가 됐다:\n    " +
                            string.Join("\n    ", 자리들));
            else if (자리들.Count < 지금)
                Debug.Log($"[폐기 이름] {갈래}가 {지금} -> {자리들.Count}개로 줄었다. " +
                          "상한도 같이 내려 주면 다음 사람이 되살리는 것을 더 촘촘히 막는다.");
        }

        Assert.IsEmpty(늘어난것,
            "폐기된 개념을 가리키는 라틴 이름이 늘었다. 낱말 게이트는 한글만 보므로 " +
            "이런 이름은 초록불 뒤에 숨는다 — 실제로 폐기된 기상 장치가 그렇게 " +
            "첫 화면에서 돌고 있었다:\n  " + string.Join("\n  ", 늘어난것));
    }

    /// <summary>
    /// <b>보고용.</b> 갈래별로 몇 개가 남았는지와 그 이름들을 콘솔에 적는다.
    /// <b>그 수가 곧 다음 라운드의 작업량이다</b> — 이름을 고치는 일은 씬·프리팹을
    /// 고치는 일이라 이 라운드가 하지 않았다. 늘 초록이므로 아무도 막지 않는다.
    /// </summary>
    [Test]
    public void 폐기된_라틴_이름이_남은_자리를_센다()
    {
        var 줄 = new List<string> { "갈래\t자리\t상한" };
        var 모두 = new HashSet<string>();
        ...
        foreach (var (갈래, 말들, 지금) in 폐기된_라틴_이름)
        {
            ...
            줄.Add($"{갈래}\t{자리들.Count}\t{지금}");
            foreach (var 자리 in 자리들) 줄.Add("    └ " + 자리);
        }

        줄.Add($"합계(중복 뺀 이름)\t{모두.Count}");
        줄.Add($"훑은 이름 전체\t{AssetTextScan.이름들().Count}");
        Debug.Log("[폐기 이름 현황]\n" + string.Join("\n", 줄));
    }

    /// <summary>
    /// <b>훑개가 이름을 정말로 읽는다.</b> 정규식 하나가 어긋나면 이름을 한 개도
    /// 못 읽게 되고, 그때 위의 게이트는 <b>0건이라 초록</b>이 된다 — 눈이 먼
    /// 게이트가 통과한 것과 정말 깨끗한 것을 구별할 수 없다.
    ///
    /// 그래서 <b>지금 살아 있는 이름 하나</b>를 실제로 집어 온다. 이 이름이
    /// 언젠가 바뀌면 이 검사가 빨개지는데, 그때는 다른 살아 있는 이름으로
    /// 옮기면 된다 — 조용히 무의미해지는 것보다 낫다.
    /// </summary>
    [Test]
    public void 이름_훑개가_비어_돌지_않는다()
    {
        var 이름들 = AssetTextScan.이름들();
        Assert.Greater(이름들.Count, 100,
            "씬과 프리팹에서 읽어 온 이름이 너무 적다. 정규식이나 검사 범위가 깨졌다");

        Assert.IsTrue(이름들.Any(n => n.이름 == "Campfire"),
            "살아 있는 프리팹 이름 Campfire를 못 읽었다. 훑개가 눈이 멀었다");

        Assert.IsTrue(이름들.Any(n => n.자리.StartsWith("Assets/01.Scenes/")),
            "씬에서 읽어 온 이름이 하나도 없다");
        Assert.IsTrue(이름들.Any(n => n.자리.StartsWith("Assets/05.Prefabs/")),
            "프리팹에서 읽어 온 이름이 하나도 없다");
    }

    /// <summary>
    /// <b>이름 금지어마다 음성 확인.</b> 위 본문 게이트와 같은 이유 —
    /// 조각을 잇다 실수하면 아무것도 물지 않는 말이 생기고,
    /// 그것은 0건과 구별되지 않는다.
    /// </summary>
    [Test]
    public void 이름_금지어마다_실제로_무는지_확인한다()
    {
        var 이상한것 = new List<string>();

        foreach (var (갈래, 말들, _) in 폐기된_라틴_이름)
        {
            Assert.IsNotEmpty(말들, $"[{갈래}]에 말이 하나도 없다");

            foreach (var 말 in 말들)
            {
                if (string.IsNullOrWhiteSpace(말) || 말.Length < 4)
                {
                    이상한것.Add($"[{갈래}] 「{말}」 — 조각을 잇다 만 것 같다");
                    continue;
                }

                if (!AssetTextScan.걸리는가("Old_" + 말 + "_Thing", 말))
                    이상한것.Add($"[{갈래}] 「{말}」 — 자기가 들어 있는 이름도 못 잡는다");

                if (AssetTextScan.걸리는가("GameObject (3)", 말))
                    이상한것.Add($"[{갈래}] 「{말}」 — 아무 이름이나 잡는다");
            }
        }
    ...
    /// <summary>
    /// <b>살아 있는 이름은 안 문다.</b> 잘못 잡는 게이트는 없느니만 못하다는
    /// 규율이 여기에도 그대로 산다.
    ///
    /// 표본은 <b>지금 저장소에 실제로 있는 이름</b>들이다 — 지어낸 이름으로
    /// 확인하면 "안 문다"가 저장소와 무관한 말이 된다.
    /// </summary>
    [Test]
    public void 살아_있는_이름은_폐기_이름에_걸리지_않는다()
    {
        var 살아있는이름 = new[]
        {
            "GlobalVolume_Surface",   // 「지표」는 살아 있는 말이다
            "Drop_AlienAlloy_Mat",    // alien_alloy 아이디는 살아 있다
            "Node_MacroniumQuartz",
            "Campfire", "Lantern", "SkyDisc", "Terrain",
            "Piece_Wall", "Snap_Plat_X+", "Trigger_Surveyed",
            "LandingSite",            // 착륙선은 살아 있다. 폐기는 「부서져 도착」 쪽이다
        };

        var 오탐 = new List<string>();
        foreach (var 이름 in 살아있는이름)
            foreach (var (갈래, 말들, _) in 폐기된_라틴_이름)
                foreach (var 말 in 말들)
                    if (AssetTextScan.걸리는가(이름, 말))
                        오탐.Add($"[{갈래}] 「{말}」이 살아 있는 이름을 물었다: \"{이름}\"");

        Assert.IsEmpty(오탐,
            "살아 있는 이름이 폐기 이름에 걸린다. 금지어를 좁히거나 갈래를 나눠라:\n  " +
            string.Join("\n  ", 오탐));

        // 짝: 그 이름들이 정말로 저장소에 있어야 "안 문다"가 뜻을 갖는다.
        var 있는이름 = new HashSet<string>(AssetTextScan.이름들().Select(n => n.이름));
        foreach (var 이름 in new[] { "GlobalVolume_Surface", "Drop_AlienAlloy_Mat", "Campfire" })
            Assert.IsTrue(있는이름.Contains(이름),
                $"{이름}이 저장소에서 사라졌다. 음성 확인의 표본을 살아 있는 것으로 옮겨라");
    }

    // ── 도구 ────────────────────────────────────────────────

    /// <summary>한 갈래의 말들이 걸린 이름 자리. 같은 이름을 두 번 세지 않는다.</summary>
    static List<string> 이름이_걸린_자리(string[] 말들) =>
        AssetTextScan.이름을_찾는다(말들).Distinct().OrderBy(자리 => 자리).ToList();
