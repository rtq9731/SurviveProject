...
public class AiSpeechGateTests
{
    ...
    ///
    /// <b>맨낱말 "회수"는 넣지 않았다.</b> 프롤로그의 "기체는 회수 불가 상태입니다"가
    /// 걸린다. 금지하려는 것은 회수라는 낱말이 아니라 <b>"저 개체가 잔해를 거둬
    /// 간다"는 판정</b>이라, 그 판정이 되는 꼴만 막는다.
    ///
    ...
    public void 훑는_자리에_진짜_AI의_말이_들어_있다()
    {
        ...
        Assert.IsTrue(말들.Any(m => m.글.Contains("해당 구조를 적용한 제작법을 확보했습니다")),
            "연구 대사를 못 담았다");
        Assert.IsTrue(말들.Any(m => m.글.Contains("비상 착륙 완료")), "연출 자막을 못 담았다");
    ...
    // 상황 통보라 세 문장 형식을 요구할 수 없다 — 요구하면 "비상 착륙 완료"가
